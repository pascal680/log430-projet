...
- **Contention sous forte charge** : les tests de stress (200 VUs, 1 compte partagé) ont révélé une contention sur les verrous de lignes InnoDB dans `bank_transactions`. Le taux de succès des transactions tombe à ~20 % dans ce scénario artificiel (tous les VUs sur le même compte). Avec 20 comptes isolés (1 par groupe de VUs), la contention est répartie et la limite devient la capacité du pool HikariCP (50 connexions). Solution : scaling horizontal de `payment-service` + 2e instance en mode LB.
